...
class Point(tk.Tk):
    ...
    def __init__(self):
        ...
        #a = tk.Label(self, text="⭕", font=("Moderne Sans",15), fg="#0f0", bg=self.chroma)
        #a.pack()

    def createPoint(self, x, y):
        #a = tk.Label(self, text="T", font=("Moderne Sans",15), fg="#0f0")#, bg=self.chroma)
        #a.pack()
        ...
        #self.wm_attributes("-transparentcolor", self.chroma)
...
class App:
    ...
    def createCircle(self, pos):
        ...
        self.point = Point()
        self.point.mainloop()
        self.point.createPoint(pos[0], pos[1])
...
# THE ACTUAL AUTOCLICKER
    ...
    def on_press(self, key):
        if self.started:
            ...
            elif key == self.pos_key:
                print("current mouse pos is:", self.mouse.position)
                self.createCircle(self.mouse.position)
